...
export const SYM_NAME: Record<SymId, string> = {
  ...
  [Sym.ROOKIE]: 'ROOKIE',
  [Sym.REF]: 'REF',
  // Ключ перечисления остаётся CHAMPION по той же причине, что и DUKE ниже, —
  // это внутренний идентификатор. Игрок видит TICKET: символ рисует билет
  // «300$», и называть билет чемпионом было странно.
  [Sym.CHAMPION]: 'TICKET',
  // Ключ перечисления остаётся DUKE — это внутренний идентификатор мат-модели,
  // трогать его означало бы переписывать core и полсотни ссылок в тестах ради
  // чисто визуального переименования. Видимое игроку имя персонажа — VAN,
  // и меняется оно только здесь и в подписях файлов ассетов.
  [Sym.DUKE]: 'VAN',
  [Sym.WILD]: '♂ WILD',
